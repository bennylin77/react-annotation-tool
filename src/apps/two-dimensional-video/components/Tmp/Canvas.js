...
*/
	...
	render() {
		const { height, width, objects, played, focusing, adding, entities, annotations, checkEmpty} = this.props;
		...
		annotations.slice().reverse().forEach( ann =>{
			...
			for( let i = 0; i < trajectories.length; i++){
				...
				if(played >= trajectories[i].time){
					...
					const warningText = checkEmpty && trajectories.length < 2 && <Text offsetY={-5} x={0} y={height} fontFamily={'Calibri'} text={'You should track the cell bound by this box'} fontSize={16} lineHeight={1.2} fill={'#fff'} ></Text>
					...
					layerItems.push(<Group x={x} y={y} key={name} id={id} name={name} draggable={isManipulatable} onDragMove={this.handle} onMouseDown={this.handleGroupMouseDown} onDragEnd={this.handleGroupDragEnd} onDragStart={this.handleGroupDragStart} onDragMove={this.handleGroupDragMove}>{labelText}{rect}{dots}{warningText}</Group>)
					break;
				}
			}
		})

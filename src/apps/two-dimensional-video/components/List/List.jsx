...
class List extends Component {
	...
	handleDelete = (name) => {
		...
		this.setState((prevState, props) => {
			...
			return {
				checkbox: false, disableDeleteModal: prevState.checkbox, modalDeleteName: '', modal: false, modalMessage: '', modalTitle: '',
			};
	...
	handleSplit = () => {
		...
		this.setState((prevState, props) => {
			...
			return {
				checkbox: false, disableSplitModal: prevState.checkbox, modalSplitName: '', modal: false, modalMessage: '', modalTitle: '',
			};
	...
	handleShowHide = () => {
		...
		this.setState((prevState, props) => {
			...
			return {
				checkbox: false, disableShowHideModal: prevState.checkbox, modalShowHideData: null, modal: false, modalMessage: '', modalTitle: '',
			};
	...
	handleModalToggle = () => {
		this.setState({
			modal: !this.state.modal, checkbox: false, modalShowHideData: null, modalDeleteName: '', modalSplitName: '', modalMessage: '', modalTitle: '',
		});
	}
	...
	render() {
		const {
			objects, duration, played, focusing, height, entities, annotations, trajectoryCollapses, checkEmpty,
		} = this.props;
		const items = [];
		const sortedAnn = this.sortAnnotations();
		// console.log(sortedAnn)
		sortedAnn.forEach((ann) => {
			if (!entities.annotations[ann].isManipulatable) { return; }
			...
			const children = [];
			for (const c of entities.annotations[ann].children) { children.push(<span key={ c } onClick={ () => this.handleAnnotationClick(c) } className='video-ann-relatives'>{`${entities.annotations[c].label} `}</span>); }
			const { color } = entities.annotations[ann];

			let split; let show; let
				hide;
			show = (
				<Button outline className='d-flex align-items-center video-ann-button' onClick={ () => this.handleShowHideModal({ name, status: SHOW }) }>
					<IoMdEye />
					{' '}
					{SHOW}
				</Button>
			...
			for (let i = 0; i < trajectories.length; i++) {
				...
				if (trajectories[i].time === played) { trajectoryStyle.color = 'rgb(33, 37, 41)'; }
				trajectoryItems.push(<ListGroupItem key={ trajectories[i].time } className='trajectory-item d-flex align-items-center justify-content-between'>
															 <div className='trajectory d-flex justify-content-between' style={ trajectoryStyle } onClick={ () => this.handleTrajectoryJump({ name, time: trajectories[i].time }) }>
																 <div className='trajectory-status pr-1'>
							<b>{trajectories[i].status}</b>
							{' '}
at
...
{' '}
				...
				let arrow;
				if (i !== trajectories.length - 1 && played > trajectories[i].time && played < trajectories[i + 1].time) { arrow = <ListGroupItem key={ trajectories[i].time + 1 } className='trajectory-item'><FaArrowDown style={ { color: '', fontSize: '1em' } } /></ListGroupItem>; } else if (i === trajectories.length - 1 && played > trajectories[i].time) { arrow = <ListGroupItem key={ trajectories[i].time + 1 } className='trajectory-item'><FaArrowDown style={ { color: '', fontSize: '1em' } } /></ListGroupItem>; } else if (i !== trajectories.length - 1 && played === trajectories[i + 1].time) { arrow = <ListGroupItem key={ trajectories[i].time + 1 } className='trajectory-item'><FaArrowDown style={ { color: '', fontSize: '1em' } } /></ListGroupItem>; } else { arrow = ''; }
				...
				if (played >= trajectories[i].time) {
					...
					if (trajectories[i].status === SHOW) {
						hide = (
							<Button outline className='d-flex align-items-center video-ann-button' onClick={ () => this.handleShowHideModal({ name, status: HIDE }) }>
								<IoMdEyeOff />
								{' '}
								{HIDE}
							</Button>
						...
						split = (
							<Button outline className='d-flex align-items-center video-ann-button' onClick={ () => this.handleSplitModal(name) }>
								<MdCallSplit />
								{' '}
								{SPLIT}
							</Button>
					...
					if (trajectories[i].status === SPLIT) { show = ''; }
				}
	    	}
				...
				items.push(<ListGroupItem className='video-ann video-ann-highlight' key={ name } name={ name } style={ { borderColor: color.replace(/,1\)/, ',.3)') } }>
					...
					<div>
						{parentAnn ? (
							<div>
								{' '}
								<Badge color='secondary'>Parent</Badge>
								{' '}
								<span onClick={ () => this.handleAnnotationClick(parentAnn.name) } className='video-ann-relatives'>{entities.annotations[parentAnn.name].label}</span>
							</div>
						) : '' }
					</div>
					<div>
						{children.length > 0 ? (
							<div>
								<Badge color='secondary'>Children</Badge>
								{' '}
								{children}
							</div>
						) : '' }
		...
		if (items.length == 0) { return (<div className='d-flex align-items-center justify-content-center' style={ { height: height - 60 } }>Click the button above to begin tracking a new cell </div>); }
		return (
			<div>

...
import {
	Button, ButtonGroup, ListGroup, ListGroupItem, Collapse,
} from 'reactstrap';
...
class List extends Component {
	...
	render() {
		const {
			annotations, focusing, dynamicOptions, disabledOptionLevels, entities, optionRoot, height,
		} = this.props;
		const items = [];
		annotations.forEach((ann) => {
			const { selected } = entities.annotations[ann];
			const { color } = entities.annotations[ann];
			const { closed } = entities.annotations[ann];
			if (ann === focusing) {
				items.unshift(<ListGroupItem className='object-item object-item-highlight' key={ ann } name={ ann } style={ { borderColor: color.replace(/,1\)/, ',.3)') } }>
						...
						<h5 className='object-item-title mr-auto'>
